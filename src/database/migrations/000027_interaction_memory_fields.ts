...
-- Drop memory_id from conversation_messages (now lives on interactions table)
-- Use rename-recreate pattern since SQLite doesn't support DROP COLUMN

-- Create new table without memory_id but keeping interaction_id
...
-- Copy data from old table (exclude memory_id)
...
ALTER TABLE interactions ADD COLUMN memory_id TEXT;
ALTER TABLE interactions ADD COLUMN continued_interaction_id TEXT;
`;
