...
export async function deleteCharacterAIProviderConfig(id: string): Promise<void> {
  const db = getDatabase();
  if (await isCharacterAIProviderConfigInUse(id)) throw new Error(`CharacterAI provider config ${id} is in use and cannot be soft deleted`);
  return withTransaction(db, async (tx) => {
