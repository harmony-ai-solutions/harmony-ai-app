...
export async function getImaginationConfig(id: number, includeDeleted = false): Promise<ImaginationConfig | null> {
  ...
  const query = includeDeleted
    ? 'SELECT id, name, provider, provider_config_id, deleted_at FROM imagination_configs WHERE id = ?'
    : 'SELECT id, name, provider, provider_config_id, deleted_at FROM imagination_configs WHERE id = ? AND deleted_at IS NULL';
...
export async function getImaginationConfigByName(name: string, includeDeleted = false): Promise<ImaginationConfig | null> {
  const db = getDatabase();

  const query = includeDeleted
    ? 'SELECT id, name, provider, provider_config_id, deleted_at FROM imagination_configs WHERE name = ?'
    : 'SELECT id, name, provider, provider_config_id, deleted_at FROM imagination_configs WHERE name = ? AND deleted_at IS NULL';
...
export async function getAllImaginationConfigs(includeDeleted = false): Promise<ImaginationConfig[]> {
  const db = getDatabase();

  const query = includeDeleted
    ? 'SELECT id, name, provider, provider_config_id, deleted_at FROM imagination_configs ORDER BY name'
    : 'SELECT id, name, provider, provider_config_id, deleted_at FROM imagination_configs WHERE deleted_at IS NULL ORDER BY name';

  const [results] = await db.executeSql(query);

  const configs: ImaginationConfig[] = [];
  for (let i = 0; i < results.rows.length; i++) {
    const row = results.rows.item(i);
    configs.push({
      id: row.id,
...
export async function updateImaginationConfig(config: ImaginationConfig): Promise<void> {
  const db = getDatabase();
  ...
  return withTransaction(db, async (tx) => {
    ...
    if (result.rowsAffected === 0) {
      throw new Error(`Imagination config not found: ${config.id}`);
    }
  });
}
