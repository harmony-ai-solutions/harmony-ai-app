...
- The Characters screen **＋ button** now opens a "New AI Partner" sheet instead of a speed-dial menu. It asks how you'd like to create the partner — **New AI Partner** (opens the Create AI Partner wizard), **From an Existing One** (picks an existing character profile, then opens the Create AI Partner wizard with that profile linked), or **Import Character Card** (existing PNG/JSON import).
