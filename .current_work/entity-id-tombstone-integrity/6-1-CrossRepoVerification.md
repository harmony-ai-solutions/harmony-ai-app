...
- `user` entity + its mirrors (exempt, mirrors renamed),
...
post-rewrite — accept + log, review-5)**. **D62 (review 5): the expected old→new map is generated BY
...
3. App release lands after the engine: one-time wipe runs (3-2, checked inside `initiateSync` per D38) →
...
7. **Wipe gate verified (D58 — review 4):** the labeled rebuild screen ("Rebuilding from Soulbits
   Engine…", i18n) shows for the duration of the wipe + first pull, then clears; no screen renders
...
2. Delete "Isabella" (tombstone both sides; verify NO purge on app finalize — 4-1).
...
6. **Restore round-trip from the app (5-3, D41):** delete an entity → restore it in the app's Deleted
   entities screen (local resurrect) → sync push → engine 1-1 resurrect branch fires (children resurrected,
   D24 teardown, cache refresh) → FE and a second device see the entity live again. Assert **no duplicate
   greeting** over restored history (D9), complete child resurrection on both sides, and that the app's
   payload carried `deleted_at: null` with a strictly-newer `updated_at` (the resurrect trigger).
...
- [ ] App-initiated restore round-trip verified (5-3: local resurrect → sync push → engine 1-1 resurrect →
      FE/second device live; no duplicate greeting)
