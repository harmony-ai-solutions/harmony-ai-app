...
> engine FE) + live-DB spot verification + user rulings **D61–D67** (below). Most consequential: **D61 moves
...
| # | Decision |
|---|---|
| D1 | **Entity IDs are never erased & never reused.** Deletion = soft-delete (tombstone); tombstones are **never purged** from either DB. This is the original design invariant and is restored/protected by this plan. |
...
| D9 | **Resurrect ≡ restore; greeting guard stays live-only** (user-ruled). The 1-1 sync-apply resurrect runs the **same** child-resurrection logic as 5-1's restore (shared repository helper keyed on the tombstone's pre-overwrite `deleted_at`). `HasPriorConversation` keeps filtering `deleted_at IS NULL` — tombstoned history is never visible, so a live-only guard is correct; **restore completeness** (children live again) is what suppresses duplicate greetings. Do NOT make the guard tombstone-aware (would silently empty "clear chat"-style restarts). |
...
| D31 | **Seed ids are exempt built-ins** like `user`: `claire` (`config/db/init.go:132`) and `default-user-profile` (`:289`) keep their raw ids, documented; no per-install derivation churn. |
...
| # | Decision |
|---|---|
| D41 | **App-side restore UI in scope (parity with 5-2).** New phase **5-3**: a `DeletedEntitiesScreen` (the `DisabledAIsScreen` pattern — settings sub-screen, both entity types in one list) + `listDeletedEntities`/`restoreEntity` repo helpers. Restore = **local resurrect** (`deleted_at = NULL`, D29-style `updated_at` bump, children resurrected via the D17 instant-equality matcher) that propagates via the **normal sync upload** — the engine's 1-1 resurrect branch (children + D24 teardown + cache refresh) makes it live engine-side; no management-API access from the app, no new sync protocol message (D4's "propagates via sync" taken literally). Prerequisite parity fixes ride in 5-3: the app delete cascade grows to D26's **8** children (+`chat_conversation_settings`, `+lifecycle_state`) and `deleteUserPersona` adopts D17's one-captured-`now` stamping. Restore carries an **alias-collision guard** (re-suffix via the alias-copy helper when a live twin holds the alias — the engine's partial unique index would otherwise abort the apply; the same guard is added to 5-1's restore helper). 5-2's app-restore non-goal note is superseded. |
...
| Phase | Scope | Repo(s) |
|---|---|---|
| 1 | Engine integrity: ghost-aware sync apply (N1), tombstone-purge removal, sync observability & session hygiene | harmony-link |
...
| 4 | App integrity: stop tombstone purge, surface sync failures, fix stuck "Connecting…" UX, gate hard deletes | harmony-ai-app |
| 5 | Deletion strategy: restore API + Deleted-entities UI — engine FE **and app** (D41) | harmony-link, engine FE, app |
...
the incident; verbatim-timestamp apply from 1-1 ships here as a correctness fix), Phase 2 next, then
...
then 4-2, Phase 5 (**5-1/5-2 engine first, then 5-3 app restore** — it depends on 1-1's resurrect branch and
4-1's tombstone retention), Phase 6.
...
- [ ] **Phase 1: Engine Integrity (sync apply & purge)**
  - [ ] Ghost-Aware Sync Apply — N1 Fix ([1-1-EngineGhostAwareSyncApply.md](1-1-EngineGhostAwareSyncApply.md))
  - [ ] Tombstone Purge Removal / Repair ([1-2-EngineTombstonePurgeRemoval.md](1-2-EngineTombstonePurgeRemoval.md))
...
- [ ] **Phase 4: App Integrity & UX**
  - [ ] Stop hard-purging never-erase tables ([4-1-AppTombstonePurgeRemoval.md](4-1-AppTombstonePurgeRemoval.md))
...
- [ ] **Phase 5: Restore Feature (Deletion Strategy)**
  - [ ] Engine: deleted-list + restore management API ([5-1-EngineRestoreApi.md](5-1-EngineRestoreApi.md))
  - [ ] Engine FE: Deleted entities view + restore + copy fixes ([5-2-EngineFeDeletedViewRestore.md](5-2-EngineFeDeletedViewRestore.md))
  - [ ] App: Deleted entities view + restore ([5-3-AppDeletedViewRestore.md](5-3-AppDeletedViewRestore.md))
...
- **Migration numbering:** both repos' highest migration is **000044** — the id-pattern migration is **000045**
  on each side (plan's older "000001–000042" references were stale).
...
- **5-3 depends on 1-1 and 4-1** (engine resurrect branch + local tombstone retention): app-initiated restore
  is silently dropped by the engine's pre-1-1 deleted-filtered apply — sequence 5-3 after Phase 1 / 4-1 land
  (shipping it with Phase 5 is fine).
- **Known accepted divergence (D67):** engine memory consolidation hard-deletes source rows after merging
  ...
  Documented exception to D1 — revisit only if memory duplication becomes user-visible.
