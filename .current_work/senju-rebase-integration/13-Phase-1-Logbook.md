...
- **Review-phase updates (commit `755c117`, see `senju-followup-phase1/10-ReviewCorrections.md`)**: several Phase-2 pointer items above are now **stub-supported** (still need their real backend counterparts): profile→listing linkage (`sourceProfileId` + `getListingForProfile`/`isChatLocked` — the marketplace PREVIEW LOCK, user ruling: viewable free, chat locked until acquired, own library never locked; backend must enforce server-side); text/theme publish + listing edit + re-list + library-remove (all flow through stub APIs); `salesCount` (on the summary wire). Acquire→AIProfile deep link remains backend-future (commented TODO preserved). NEW seams: `ProfileExtrasService` (username/bio/avatar per-device, §5 swap target) + `VisibilitySettingsSection` component (backend needs a per-listing visibility/classification field). Dev-DB wipe (standing question #2) CONFIRMED on-device 2026-08-26.

---

## Review phase — post-Phase-1 corrections (2026-08-26, commit `755c117`)

User code review of the phase-1 diff (baseline `59739e0`) found UI removed beyond the "screens stay verbatim" goal. Rulings + implementations (full detail: `senju-followup-phase1/10-ReviewCorrections.md`):

- **Restored on extended stubs**: marketplace detail preview region (image/teaser/unlock hint) + `salesCount`; publish/edit/re-list/text+theme flows (`updateListing`/`relistListing`/`removeLibraryEntry`, draft `kind/text/previewText/sourceProfileId`; fixed latent unreachable theme step); marketplace preview chat locks (central `openCharacterChat` gate + AIProfile price pill/lock live + ChatDetail defense-in-depth); EditProfile username/bio/avatar + MyProfile header extras (NEW `ProfileExtrasService`, old `@harmony_profile/<userId>` key reused).
- **New component**: `VisibilitySettingsSection` (private/public/marketplace + SOUL price) — extracted from the old CreateAI section per ruling, wired into the publish screen; documented in 20-Backend-Concept §1.
- **Preserved as commented code**: acquire→AIProfile deep link (`TODO(backend)` — needs profile materialization).
