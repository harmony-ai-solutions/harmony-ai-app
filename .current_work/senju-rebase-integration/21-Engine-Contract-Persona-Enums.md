...
| A19 | **Q5 SUPERSEDED (user ruling 2026-09-01): favorites become an `is_favorite` column on `character_profiles`; the `character_favorites` sidecar table is DROPPED both sides (paired 000044, ALTER + in-place data carry).** The user explicitly accepts the row-LWW coupling this creates: a favorite toggle bumps the profile row's `updated_at` and rides full-row profile sync, so a concurrent profile edit on another device can lose LWW against a favorite toggle (and vice versa) — acceptable because favorites and profile edits rarely happen on different devices before a sync. Secondary accepted effects: every toggle re-syncs the full profile row; favorite-recency ordering (`created_at DESC` of the sidecar) is lost (no consumer needed it — CharactersScreen builds a membership Set). `is_favorite INTEGER NOT NULL DEFAULT 0` rides as JSON number 0/1 (same convention as `pinned`/`archived`). Removed with the table: the 4-1/4-2 favorites registration (upload list, TABLE_ORDER, PK-registry entry, backfill registry, FK-diagnostics, apply case, models/repos both sides). Allowlist unchanged (favorites was a matched table, never allowlisted; the identical-ALTER pair keeps `character_profiles` byte-identical — parity gate re-verified 2026-09-01: 55/55 match, exactly the 3 §9-A14 Go-only entries). |
